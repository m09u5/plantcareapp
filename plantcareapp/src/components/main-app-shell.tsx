...
export function MainAppShell() {
  ...
  return (
    <View style={styles.appRoot}>
      {activeScreen === 'plants' ? <PlantsScreen /> : <PlaceholderScreen title={activeScreen} />}
...
function PlantsScreen() {
  ...
  async function savePlant(draft: DraftPlant) {
    ...
    if (token && draft.id && draft.id > 0) {
      const savedPlant = await updatePlant(token, draft.id, draft.name, draft.interval);
      setPlants((currentPlants) =>
        ...
        <Pressable style={styles.modalCard}>
          ...
          <View style={styles.sliderRow}>
            ...
            <IntervalSlider
              ...
              onChange={(interval) =>
                setLocalDraft((current) => (current ? { ...current, interval } : current))
              }
            />
          </View>
...
function IntervalSlider({ interval, onChange }: { interval: number; onChange: (value: number) => void }) {
  ...
  return (
    <View
      ...
      <View style={[styles.sliderBadge, { left: Math.max(0, Math.min(sliderWidth - 56, percent * sliderWidth - 28)) }]}>
...
function PlaceholderScreen({ title }: { title: string }) {
  ...
  return (
    <View style={styles.placeholderScreen}>
      <Text style={styles.screenTitle}>{title === 'scan' ? 'Scan :' : 'Settings :'}</Text>
      {title === 'settings' ? (
